    ...
    (let [stateful-data (o/get r vnode-stateful-key)
      ...
      (when (not (dirty-component? vnode))
        ...
        (when-not (aget render-queue index-render-queue-dirty-flag)
          ...
          (if async
            (run-later (process-render-queue render-queue))
...
(defn- insert-vnode-before* [parent-node vnode ref-node]
  (if (component? vnode)
    ...
    (let [children (.getChildren parent-node)
      ...
      (if (== index -1)
        (.add children (aget vnode index-node))
        (.add children (.indexOf children ref-node) (aget vnode index-node))))))
    ...
    (if (> *new-node* 0)
      (let [state-ref (atom nil)
        ...
        (aset *vnode* index-comp-data
              #js[component-name state-ref nil vnode-index *component-depth*])
